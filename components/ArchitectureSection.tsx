...
const nodes: ArchNode[] = [
  // Column 1
  { id: "user", label: "User Input", sublabel: "Voice · Text · Vision · Screen", icon: "👤", color: "#8b5cf6", x: 120, y: 220 },
  // Column 2
  { id: "overlay", label: "Android Overlay", sublabel: "1,800+ lines Kotlin · SoraEyes", icon: "📱", color: "#a78bfa", x: 350, y: 80 },
  { id: "nodejs", label: "Node.js Backend", sublabel: "Express · Socket.IO · 13 Services", icon: "⚡", color: "#d946ef", x: 350, y: 220 },
  { id: "dashboard", label: "Web Dashboard", sublabel: "Global Centralized Control", icon: "📊", color: "#9333ea", x: 350, y: 360 },
  // Column 3
  { id: "gemini", label: "Gemini Intelligence", sublabel: "2.5 Flash · Waterfall · Tools", icon: "🧠", color: "#8b5cf6", x: 580, y: 80 },
  { id: "pinecone", label: "Pinecone RAG", sublabel: "768-dim Vectors · Long-term", icon: "🔍", color: "#d946ef", x: 580, y: 220 },
  { id: "services", label: "Ecosystem Services", sublabel: "WhatsApp · TTS · Image Gen", icon: "🌐", color: "#9333ea", x: 580, y: 360 },
];
...
const connections: Connection[] = [
  { from: "user", to: "overlay", path: "M 210 220 C 240 220, 240 80, 260 80", color: "#8b5cf6" },
  { from: "user", to: "nodejs", path: "M 210 220 L 260 220", color: "#d946ef" },
  { from: "overlay", to: "nodejs", path: "M 350 120 L 350 180", color: "#a78bfa" },
  { from: "nodejs", to: "dashboard", path: "M 350 260 L 350 320", color: "#9333ea" },
  { from: "nodejs", to: "gemini", path: "M 440 220 C 470 220, 470 80, 490 80", color: "#8b5cf6" },
  { from: "nodejs", to: "pinecone", path: "M 440 220 L 490 220", color: "#d946ef" },
  { from: "nodejs", to: "services", path: "M 440 220 C 470 220, 470 360, 490 360", color: "#9333ea" },
  { from: "pinecone", to: "gemini", path: "M 580 180 L 580 120", color: "#a78bfa" },
];
...
function PulsingDot({ cx, cy, color, delay }: { cx: number; cy: number; color: string; delay: number }) {
  return (
    <>
      ...
      />
      <motion.circle
        cx={cx} cy={cy} r={3} fill="none" stroke={color} strokeWidth={1}
...
function ArchitectureNode({ node, delay, isInView }: { node: ArchNode; delay: number; isInView: boolean }) {
  return (
    <motion.foreignObject
      x={node.x - 90} y={node.y - 40} width={180} height={80}
...
export default function ArchitectureSection() {
  ...
  return (
    <section id="architecture" className="relative py-24 md:py-32">
      ...
      <div className="absolute top-1/2 right-0 w-[500px] h-[500px] bg-fuchsia-600/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <ScrollReveal className="mb-20 hidden lg:block">
          <div ref={svgRef} className="relative glass-card p-8 overflow-hidden">
            {/* Inner glow */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[200px] bg-violet-500/5 rounded-full blur-[80px] pointer-events-none" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {techCategories.map((cat, i) => (
              <ScrollReveal key={cat.title} delay={i * 0.1}>
                {/* subcard-hover for premium GPU-smooth expand */}
                <div className="glass-card p-6 h-full subcard-hover border border-white/[0.06]">
